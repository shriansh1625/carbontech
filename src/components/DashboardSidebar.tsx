...
export default function DashboardSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-16 left-0 h-[calc(100vh-4rem)] w-64 bg-[#0d1512]/95 backdrop-blur-xl border-r border-emerald-500/10 z-40 transition-transform duration-300 ${
      ...
      >
        <div className="p-4 border-b border-emerald-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-white text-sm">{profile?.displayName || 'User'}</p>
              <p className="text-xs text-emerald-400/70 capitalize">{role}</p>
            </div>
          </div>
        </div>
        <nav className="p-3 space-y-1">
          {menu.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`text-lg ${active ? 'text-emerald-400' : ''}`} />
                {item.label}
              </Link>
            );
          })}
